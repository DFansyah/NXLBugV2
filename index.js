...
const { default: NXL, delay, PHONENUMBER_MCC, makeCacheableSignalKeyStore, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, Browsers} = require("@whiskeysockets/baileys")
   ...
   if (pairingCode && !NXL.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +62xxxx")))
            process.exit(0)
         }
      } else {
    ...
    NXL.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            const m = smsg(NXL, mek, store)
            require("./case")(NXL, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    //autostatus view
        NXL.ev.on('messages.upsert', async chatUpdate => {
            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
            	await NXL.readMessages([mek.key]) }
            }
    })
...
NXL.ev.on("connection.update",async  (s) => {
        const { connection, lastDisconnect } = s
        if (connection == "open") {
        	console.log(chalk.magenta(` `))
            console.log(chalk.yellow(`🌿Connected to => ` + JSON.stringify(NXL.user, null, 2)))
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
            console.log(chalk.lime(`${themeemoji} GITHUB: WangsaffMD `))
            console.log(chalk.lime(`${themeemoji} WA NUMBER: ${owner}`))
            console.log(chalk.lime(`${themeemoji} CREDIT: ${wm}\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
    ...
    NXL.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
  let type = await NXL.getFile(path, true);
  ...
  try {
    m = await NXL.sendMessage(jid, message, { ...opt, ...options });
  } catch (e) {
    //console.error(e)
    m = null;
  } finally {
    if (!m) m = await NXL.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options });
    file = null;
    return m;
  }
}
